...
def calculate_imaginary_formula(components, element_composition, molecular_weight, total_mass=1000):
    total_moles = {'C': 0, 'H': 0, 'O': 0, 'N': 0, 'Al': 0, 'Cl': 0}  # 初始化每种元素的物质的量
    for component, percentage in components.items():
        component_mass = total_mass * percentage  # 当前成分的质量
        for element, count in element_composition.get(component, {}).items():
            ...
            total_moles[element] += element_mol  # 累加该元素的物质的量
...
def input_geatpy_isp(x1, x2, x3, x4,smiles):
    ...
    X_input_Scaler = torch.Tensor(X_input_Scaler.astype(np.float32))  # 假设 X_input_Scaler 是 NumPy 数组，转换为 Tensor
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    # print(device)
    model_isp = Model_isp_all([832,704,1024,288,64,704,384,1024])
    model_isp.load_state_dict(torch.load('isp_best_network.pth'))  # 确保模型加载到适当的设备
    ...
    return isp_train_p.cpu().detach().numpy().reshape(-1, 1)
    # return isp_train_p.cpu().detach().reshape(-1, 1)
...
def input_geatpy_cstar(x1, x2, x3, x4,smiles):
    ...
    model_cstar.load_state_dict(torch.load('cstar_best_network.pth'))  # 确保模型加载到适当的设备
    ...
    return cstar_train_p.cpu().detach().numpy().reshape(-1, 1)
...
def input_geatpy_c_t(x1, x2, x3, x4,smiles):
    ...
    model_c_t.load_state_dict(torch.load('c_t_best_network.pth'))  # 确保模型加载到适当的设备
    ...
    return c_t_train_p.cpu().detach().numpy().reshape(-1, 1)

# A = input_geatpy_isp(1,1,1,1,['CC'])  # Example usage, replace with actual values
# print(A)  # Output the result to verify the function works correctly